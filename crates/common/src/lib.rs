    ...
    impl FlagClient {
        ...
        pub async fn get(&self) -> Result<Flags, FlagError> {
            ...
            let s = self
                ...
                .send()
                .await
                .map_err(|e| FlagError::AppConfig(e.to_string()))?;
            ...
            let flags: Flags = if bytes.is_empty() {
                Flags::default()
            } else {
                serde_yaml::from_slice(&bytes).map_err(|_| FlagError::Invalid)?
            };
