    ...
    lambda_runtime::run(service_fn(move |evt: LambdaEvent<serde_json::Value>| {
        ...
        async move {
            ...
            for r in items {
                ...
                let e: Event =
                    serde_json::from_value(r).unwrap_or_else(|_| Event::UserRegistered {
                ...
                match e {
                    Event::OrderPlaced {
                        ...
                        db.put_item()
                            ...
                            .item(
                                "pk",
                                dynamodb::types::AttributeValue::S(format!("ORDER#{order_id}")),
                            )
                            .item("sk", dynamodb::types::AttributeValue::S("v0".into()))
                            .item("user_id", dynamodb::types::AttributeValue::S(user_id))
                            .item(
                                "total_cents",
                                dynamodb::types::AttributeValue::N(total_cents.to_string()),
                            )
                            .send()
                            .await?;
                    }
